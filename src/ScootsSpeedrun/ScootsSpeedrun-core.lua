local core = {
    ['init'] = function()
        ...
        ScootsSpeedrun.playerClass = {}
        local mask = CustomGetClassMask()
        
        for className, classId in pairs({
            ['DEATHKNIGHT'] = 6,
            ['DRUID'] = 11,
            ['HUNTER'] = 3,
            ['MAGE'] = 8,
            ...
            if(bit.band(mask, bit.lshift(1, classId - 1)) > 0) then
                ScootsSpeedrun.playerClass[className] = true
            end
        end
        ...
        ShouldAutoAcceptQuest = function(questId, questList)
            return questList[questId] or ScootsSpeedrun.shouldPickupQuest(questId)
        end
    ...
    ['eventHandler'] = function(_, event, arg1, arg2)
        if(event == 'ADDON_LOADED') then
            if(arg1 == 'ScootsSpeedrun') then
            ...
            if(basicConditionsMet) then
                ...
                if(map[mapIndex].conditions == nil) then
                    ...
                    local conditionChecks = {
                        ...
                        ['quest-not-handin-ready'] = ScootsSpeedrun.condition.questNotHandinReady,
                        ['quest-in-log'] = ScootsSpeedrun.condition.questInLog,
    ...
    ['shouldPickupQuest'] = function(questId)
        ...
        if(ScootsSpeedrun.extraQuests and ScootsSpeedrun.extraQuests[questId] == true) then
            return true
        end
        
        if(ScootsSpeedrun.extraQuestsOneInSet and #ScootsSpeedrun.extraQuestsOneInSet > 0) then
            for _, questSet in ipairs(ScootsSpeedrun.extraQuestsOneInSet) do
                if(questSet[questId]) then
                    local onSet = false
                
                    for questSetId, _ in pairs(questSet) do
                        if(Custom_IsOnQuest(questSetId) == 1) then
                            onSet = true
                            break
                        end
                    end
    ...
    ['buildMapFromTracker'] = function(event)
        ...
        elseif(event == 'QUEST_DETAIL' or eventLast == 'QUEST_DETAIL') then
            ...
            if(ScootsSpeedrun.shouldPickupQuest(questId)) then
                return {
                    {
                        ['action'] = 'accept-quest',
                        ['data'] = questId,
        ...
        elseif(ScootsSpeedrun.isQuestShowEvent(event)) then
            ...
            if(GetNumGossipAvailableQuests() > 0 or GetNumAvailableQuests() > 0) then
                ...
                for questIndex = 1, #availableQuests do
                    if(ScootsSpeedrun.shouldPickupQuest(availableQuests[questIndex])) then
                        table.insert(map, {
    ...
    ['buildMapFromHandinReadyQuests'] = function()
    ...
    ['applyAutoQuestAttuneables'] = function()
        ...
        if(ScootsSpeedrun.extraQuestsAttuneables) then
    ...
    ['applyAutoQuestEventQuests'] = function()
    ...
    ['processQuestStartItems'] = function()
        if(ScootsSpeedrun.options.get('auto-quest-start-items') ~= true) then
            return
        end
        
        if(ScootsSpeedrun.bankOpen) then
            return
        end
        
        ScootsSpeedrun.storage.questStartItems = ScootsSpeedrun.storage.questStartItems or {}
        local nowTime = GetTime()
        
        for bagIndex = -2, 4 do
            if(bagIndex ~= -1) then
                for slotIndex = 1, GetContainerNumSlots(bagIndex) do
                    local _, questId = GetContainerItemQuestInfo(bagIndex, slotIndex)
                    
                    if(questId and ScootsSpeedrun.extraQuestsDontAutoUseStartItem[questId] == nil) then
                        if(ScootsSpeedrun.storage.questStartItems[questId] ~= nil) then
                            ScootsSpeedrun.storage.questStartItems[questId].timestamp = nowTime
                            ScootsSpeedrun.storage.questStartItems[questId].bag = bagIndex
                            ScootsSpeedrun.storage.questStartItems[questId].slot = slotIndex
                        elseif(ScootsSpeedrun.shouldPickupQuest(questId)) then
                            ScootsSpeedrun.storage.questStartItems[questId] = {
                                ['timestamp'] = nowTime,
                                ['bag'] = bagIndex,
                                ['slot'] = slotIndex,
                                ['started'] = false,
        ...
        for questId, data in pairs(ScootsSpeedrun.storage.questStartItems) do
            if(data.timestamp < nowTime) then
                ScootsSpeedrun.storage.questStartItems[questId] = nil
            elseif(not data.started) then
                if(Custom_IsOnQuest(questId) ~= 1) then
    ...
    ['handlePopup'] = function(popup, locationId)
    ...
    ['processRegisteredEvents'] = function(event)
    ...
    ['isQuestShowEvent'] = function(event)
...
for key, func in pairs(core) do
    ScootsSpeedrun[key] = func
end
