ScootsSpeedrun.extraQuestsAttuneables = {
    -- Vanilla
    {
        ['quests'] = {
            203, -- The Second Rebellion
        },
        ['items'] = {
            4125, -- Tranquil Orb
        },
    },
    {
        ['quests'] = {
            261, -- Down the Scarlet Path
            1052, -- Down the Scarlet Path
            1053, -- In the Name of the Light
        },
        ['items'] = {
            6829, -- Sword of Serenity
            6830, -- Bonebiter
            6831, -- Black Menace
            11262, -- Orb of Lorica
        },
    },
    {
        ['quests'] = {
            307, -- Filthy Paws
        },
        ['items'] = {
            3166, -- Ironheart Chain
            3161, -- Robe of the Keeper
            3160, -- Ironplate Buckler
        },
    },
    {
        ['quests'] = {
            642, -- The Princess Trapped
            651, -- Stones of Binding
            652, -- Breaking the Keystone
            653, -- Myzrael's Allies (A)
            688, -- Myzrael's Allies (H)
            687, -- Theldurin the Lost
        },
        ['items'] = {
            4743, -- Pulsating Crystalline Shard
        },
    },
    {
        ['quests'] = {
            659, -- Hints of a New Plague?
        },
        ['items'] = {
            15455, -- Dustfall Robes
            15456, -- Lightstep Leggings
        },
    },
    {
        ['quests'] = {
            670, -- Sunken Trasure
        },
        ['items'] = {
            4550, -- Coldwater Ring
            4549, -- Seafire Band
        },
    },
    {
        ['quests'] = {
            690, -- Malin's Request
        },
        ['items'] = {
            4975, -- Vigilant Buckler
            15104, -- Wingborne Boots
        },
    },
    {
        ['quests'] = {
            713, -- Coolant Heads Prevail
            714, -- Gyro... What?
        },
        ['items'] = {
            4979, -- Enchanted Stonecloth Bracers
        },
    },
    {
        ['quests'] = {
            722, -- Amulet of Secrets
        },
        ['items'] = {
            4987, -- Dwarf Captain's Sword
        },
    },
    {
        ['quests'] = {
            939, -- Flute of Xavaric
        },
        ['items'] = {
            11873, -- Ethereal Mist Cape
            11874, -- Clouddrift Mantle
        },
    },
    {
        ['quests'] = {
            942, -- The Absent Minded Prospector
        },
        ['items'] = {
            5626, -- Skullchipper
            5627, -- Relic Blade
        },
    },
    {
        ['quests'] = {
            974, -- Finding the Source
        },
        ['items'] = {
            15789, -- Deep River Cloak
        },
    },
    -- 1000:
    {
        ['quests'] = {
            1070, -- On Guard in Stonetalon
            1085, -- On Guard in Stonetalon
            1071, -- A Gnome's Respite
            1072, -- An Old Colleague
            1073, -- Ineptitude + Chemicals = Fun
            1074, -- Ineptitude + Chemicals = Fun
            1075, -- A Scroll from Mauren
            1076, -- Devils in Westfall
            1077, -- Special Delivery for Gaxim
            1079, -- Covert Ops - Alpha
            1080, -- Covert Ops - Beta
            1091, -- Kaela's Update
            1083, -- Enraged Spirits
            1084, -- Wounded Ancients
            1082, -- Update for Sentinel Thenysil
        },
        ['items'] = {
            6678, -- Band of Elven Grace
            7334, -- Efflorescent Robe
            7335, -- Grizzly Tunic
            7336, -- Wildwood Chain
        },
    },
    {
        ['quests'] = {
            1104, -- Salt Flat Venom
            1105, -- Hardened Shells
        },
        ['items'] = {
            6732, -- Gnomish Mechanic's Gloves
        },
    },
    {
        ['quests'] = {
            1105, -- Further Instructions
        },
        ['items'] = {
            6667, -- Engineer's Cloak
            6668, -- Draftsman Boots
        },
    },
    {
        ['quests'] = {
            1140, -- The Tower of Althalaxx
            1167, -- The Tower of Althalaxx
        },
        ['items'] = {
            5614, -- Seraph's Strike
            10043, -- Pious Legwraps
        },
    },
    {
        ['quests'] = {
            1125, -- The Spirits of Southwind
            1126, -- Hive in the Tower
        },
        ['items'] = {
            18400, -- Ring of Living Stone
            18402, -- Glowing Crystal Ring
        },
    },
    {
        ['quests'] = {
            1150, -- Test of Endurance
            1151, -- Test of Strength
            1152, -- Test of Lore
            1154, -- Test of Lore
            1159, -- Test of Lore
        },
        ['items'] = {
            6804, -- Windstorm Hammer
        },
    },
    {
        ['quests'] = {
            1176, -- Load Lightening
            1178, -- Goblin Sponsorship
            1180, -- Goblin Sponsorship
            1181, -- Goblin Sponsorship
            1182, -- Goblin Sponsorship
            1183, -- Goblin Sponsorship
            1186, -- The Eighteenth Pilot
            1187, -- Razzeric's Tweaking
            1188, -- Safety First
            1189, -- Safety First
        },
        ['items'] = {
        },
    },
    {
        ['quests'] = {
            1198, -- In Search of Thaelrid
        },
        ['items'] = {
            7001, -- Gravestone Scepter
            7002, -- Arctic Buckler
        },
    },
    {
        ['quests'] = {
            1261, -- Marg Speaks
        },
        ['items'] = {
            18585, -- Band of Allegiance
            18586, -- Lonetree's Circle
        },
    },
    {
        ['quests'] = {
            1442, -- Seeking the Kor Gem
        },
        ['items'] = {
            6953, -- Verigan's Fist
        },
    },
    {
        ['quests'] = {
            1719, -- The Affray
        },
        ['items'] = {
            6975, -- Whirlwind Axe
            6977, -- Whirlwind Sword
            6976, -- Whirlwind Warhammer
        },
    },
    {
        ['quests'] = {
            1944, -- Waters of Xavian
            1945, -- Laughing Sisters
        },
        ['items'] = {
            7512, -- Nether-lace Robe
            9515, -- Nether-lace Tunic
        },
    },
    {
        ['quests'] = {
            1950, -- Get the Scoop
            1951, -- Rituals of Power
        },
        ['items'] = {
            7514, -- Icefury Wand
            11263, -- Nether Force Wand
            7513, -- Ragefire Wand
        },
    },
    -- 2000:
    {
        ['quests'] = {
            2201, -- Find the Gems
            2204, -- Restoring the Necklace
        },
        ['items'] = {
            7673, -- Talvash's Enhancing Necklace
        },
    },
    {
        ['quests'] = {
            2944, -- The Super Snapper FX
        },
        ['items'] = {
            9655, -- Seedtime Hoop
        },
    },
    {
        ['quests'] = {
            2991, -- Nekrum's Medallion
            2992, -- The Divination
            2993, -- Return to the Hinterlands
        },
        ['items'] = {
            9651, -- Gryphon Rider's Stormhammer
            9652, -- Gryphon Rider's Leggings
        },
    },
    -- 3000:
    {
        ['quests'] = {
            3371, -- Dwarven Justice
        },
        ['items'] = {
            10739, -- Ring of Fortitude
            10740, -- Centurion Legplates
            10741, -- Lordrec Helmet
        },
    },
    {
        ['quests'] = {
            3462, -- Squire Maltrake
            3463, -- Set Them Ablaze!
        },
        ['items'] = {
            10742, -- Dragonflight Leggings
            10743, -- Drakefire Headguard
            10744, -- Axe of the Ebon Drake
        },
    },
    {
        ['quests'] = {
            3520, -- Screecher Spirits
        },
        ['items'] = {
            10749, -- Avenguard Helm
            10750, -- Lifeforce Dirk
            10751, -- Gemburst Circlet
        },
    },
    {
        ['quests'] = {
            3765, -- The Corruption Abroad
        },
        ['items'] = {
            7003, -- Beetle Clasps
            7004, -- Prelacy Cape
        },
    },
    {
        ['quests'] = {
            3909, -- The Videre Elixir
            4084, -- Silver Heart
            4005, -- Aquementas
        },
        ['items'] = {
            11904, -- Spirit of Aquementas
        },
    },
    -- 4000:
    {
        ['quests'] = {
            4003, -- The Royal Rescue
        },
        ['items'] = {
            12544, -- Thrall's Resolve
            12545, -- Eye of Orgrimmar
        },
    },
    {
        ['quests'] = {
            4324, -- Yuka Screwspigot
        },
        ['items'] = {
            11865, -- Rancor Boots
            11963, -- Penance Spaulders
            12049, -- Splintsteel Armor
        },
    },
    {
        ['quests'] = {
            4496, -- Bungle in the Jungle
            4507, -- Pawn Captures Queen
        },
        ['items'] = {
            11870, -- Oblivion Orb
            11871, -- Snarkshaw Spaulders
            11872, -- Eschewal Greaves
        },
    },
    {
        ['quests'] = {
            4726, -- Broodling Essence
            4808, -- Felnok Steelspring
            4907, -- Tinkee Steamboil
            4734, -- Egg Freezing
            5522, -- Leonid Barthalomew
        },
        ['items'] = {
            15854, -- Dancing Sliver
        },
    },
    {
        ['quests'] = {
            4766, -- Mayara Brightwing
        },
        ['items'] = {
            15860, -- Blinkstrike Armguards
            15861, -- Swiftfoot Treads
        },
    },
    {
        ['quests'] = {
            4901, -- Guardians of the Altar
        },
        ['items'] = {
            16623, -- Opaline Medallion
        },
    },
    {
        ['quests'] = {
            4974, -- For The Horde!
            6569, -- Oculus Illusions
            6582, -- The Test of Skulls, Scryer
            6583, -- The Test of Skulls, Somnus
            6584, -- The Test of Skulls, Chronalis
            6585, -- The Test of Skulls, Axtroz
        },
        ['items'] = {
            16309, -- Drakefire Amulet
        },
    },
    -- 5000:
    {
        ['quests'] = {
            5001, -- Bijou's Belongings
            5002, -- Message to Maxwell
        },
        ['items'] = {
            13958, -- Wyrmthalak's Shackles
            13959, -- Omokk's Girth Restrainer
            13961, -- Halycon's Muzzle
            13962, -- Vosh'gajin's Strand
            13963, -- Voone's Vice Grips
        },
    },
    {
        ['quests'] = {
            5066, -- A Call to Arms: The Plaguelands! (A)
            5090, -- A Call to Arms: The Plaguelands! (A)
            5091, -- A Call to Arms: The Plaguelands! (A)
            10373, -- A Call to Arms: The Plaguelands! (A)
            5093, -- A Call to Arms: The Plaguelands! (H)
            5094, -- A Call to Arms: The Plaguelands! (H)
            5095, -- A Call to Arms: The Plaguelands! (H)
            10374, -- A Call to Arms: The Plaguelands! (H)
        },
        ['items'] = {
            15799, -- Heroic Commendation Medal
            15800, -- Intrepid Shortsword
            15801, -- Valiant Shortsword
            17759, -- Mark of Resolution
        },
    },
    {
        ['quests'] = {
            5047, -- Finkle Einhorn, At Your Service!
        },
        ['items'] = {
            12752, -- Cap of the Scarlet Savant
            12756, -- Leggings of Arcana
            12757, -- Breastplate of Bloodthirst
        },
    },
    {
        ['quests'] = {
            5048, -- Good Natured Emma
            5050, -- Good Luck Charm
        },
        ['items'] = {
            13473, -- Felstone Good Luck Charm
        },
    },
    {
        ['quests'] = {
            5058, -- Mrs. Dalson's Diary
        },
        ['items'] = {
            13474, -- Farmer Dalson's Shotgun
            13475, -- Dalson Family Wedding Ring
        },
    },
    {
        ['quests'] = {
            5142, -- Little Pamela
            5152, -- Auntie Marlene
            5153, -- A Strange Historian
            5154, -- The Annals of Darrowshire
            5210, -- Brother Carlin
            5211, -- Defenders of Darrowshire
            5241, -- Uncle Carlin
            5721, -- The Battle of Darrowshire
        },
        ['items'] = {
            15855, -- Ring of Protection
            15856, -- Archlight Talisman
            15857, -- Magebane Scion
        },
    },
    {
        ['quests'] = {
            5246, -- Fragments of the Past
            5247, -- Fragments of the Past
        },
        ['items'] = {
            15791, -- Turquoise Sash
            15792, -- Plow Wood Spaulders
            15795, -- Emerald Mist Gauntlets
        },
    },
    {
        ['quests'] = {
            5262, -- The Argent Hold
        },
        ['items'] = {
            13243, -- Argent Defender
            13249, -- Argent Crusader
        },
    },
    {
        ['quests'] = {
            5464, -- Menethil's Gift
            5465, -- Soulbound Keepsake
        },
        ['items'] = {
            13986, -- Crown of Caer Darrow
            14002, -- Darrowshire Strongguard
            15805, -- Penelope's Rose
            15806, -- Mirah's Song
        },
    },
    {
        ['quests'] = {
            5542, -- Demon Dogs
            5543, -- Blood Tinged Skies
            5544, -- Carrion Grubbage
            5742, -- Redemption
            5781, -- Of Forgotten Memories
            5845, -- Of Lost Honor
            5846, -- Of Love and Family
            5861, -- Find Myranda
        },
        ['items'] = {
            15411, -- Mark of Fordring
            15413, -- Ornate Adamantium Breastplate
            15418, -- Shimmering Platinum Warhammer
            15421, -- Shroud of the Exile
            16058, -- Fordring's Seal
        },
    },
    -- 6000:
    {
        ['quests'] = {
            6121, -- Lessons Anew (A)
            6122, -- The Principal Source (A)
            6126, -- Lessons Anew (H)
            6127, -- The Principal Source (H)
        },
        ['items'] = {
            15866, -- Veildust Medicine Bag
        },
    },
    {
        ['quests'] = {
            6185, -- The Eastern Plagues
        },
        ['items'] = {
            16996, -- Gorewood Bow
            16997, -- Stormrager
            16998, -- Sacred Protector
        },
    },
    -- 7000:
    -- 8000:
    {
        ['quests'] = {
            8234, -- Sealed Azure Bag
        },
        ['items'] = {
            19984, -- Ebon Mask
            20255, -- Whisperwalk Boots
            19982, -- Duskbat Drape
        },
    },
    {
        ['quests'] = {
            8578, -- Scrying Goggles? No Problem!
        },
        ['items'] = {
            21526, -- Band of Icy Depths
            21527, -- Darkwater Robes
        },
    },
    {
        ['quests'] = {
            8578, -- Scrying Goggles? No Problem!
        },
        ['items'] = {
            21523, -- Fang of Korialstrasz
            21522, -- Shadowsong's Sorrow
            21521, -- Runesword of the Red
            21520, -- Ravencrest's Legacy
        },
    },
    {
        ['quests'] = {
            8905, -- An Earnest Proposition (A | Druid)
            8913, -- An Earnest Proposition (H | Druid)
            8906, -- An Earnest Proposition (A | Hunter)
            8914, -- An Earnest Proposition (H | Hunter)
            8907, -- An Earnest Proposition (A | Mage)
            8915, -- An Earnest Proposition (H | Mage)
            8908, -- An Earnest Proposition (Paladin)
            8909, -- An Earnest Proposition (A | Priest)
            8916, -- An Earnest Proposition (H | Priest)
            8910, -- An Earnest Proposition (A | Rogue)
            8917, -- An Earnest Proposition (H | Rogue)
            8918, -- An Earnest Proposition (Shaman)
            8911, -- An Earnest Proposition (A | Warlock)
            8919, -- An Earnest Proposition (H | Warlock)
            8912, -- An Earnest Proposition (A | Warrior)
            8920, -- An Earnest Proposition (H | Warrior)
            8922, -- A Supernatural Device (A)
            8923, -- A Supernatural Device (H)
            8921, -- The Ectoplasmic Distiller
            8924, -- Hunting for Ectoplasm
            8925, -- A Portable Power Source
            8928, -- A Shifty Merchant
            8977, -- Return to Deliana
            8978, -- Return to Mokvar
            8926, -- Just Compensation (A | Druid)
            8927, -- Just Compensation (H | Druid)
            8931, -- Just Compensation (A | Hunter)
            8938, -- Just Compensation (H | Hunter)
            8932, -- Just Compensation (A | Mage)
            8939, -- Just Compensation (H | Mage)
            8933, -- Just Compensation (Paladin)
            8934, -- Just Compensation (A | Priest)
            8940, -- Just Compensation (H | Priest)
            8935, -- Just Compensation (A | Rogue)
            8941, -- Just Compensation (H | Rogue)
            8942, -- Just Compensation (Shaman)
            8936, -- Just Compensation (A | Warlock)
            8943, -- Just Compensation (H | Warlock)
            8937, -- Just Compensation (A | Warrior)
            8944, -- Just Compensation (H | Warrior)
            8929, -- In Search of Anthion (A)
            8930, -- In Search of Anthion (H)
            8945, -- Dead Man's Plea
            8946, -- Proof of Life
            8947, -- Anthion's Strange Request
            8948, -- Anthion's Old Friend
        },
        ['items'] = {
            22150, -- Beads of Ogre Might
            22149, -- Beads of Ogre Mojo
        },
    },
    -- 9000:
    {
        ['quests'] = {
            9063, -- Torwa Pathfinder
        },
        ['items'] = {
            22274, -- Grizzled Pelt
            22272, -- Forest's Embrace
            22458, -- Moonshadow Stave
        },
    },
    {
        ['quests'] = {
            9716, -- Disturbance at Umbrafen Lake
            9718, -- As the Crow Flies
            9720, -- Balance Must Be Preserved
        },
        ['items'] = {
            27734, -- Expedition Caster's Band
            27735, -- Pendant of the Marsh
            27733, -- Warden's Ring of Precision
            27730, -- Watcher's Cloak of Vigilance
        },
    },
    {
        ['quests'] = {
            9904, -- Pursuing Terrorclaw
        },
        ['items'] = {
            25924, -- Swamprunner's Boots
            25923, -- Fierce Mantle
            25922, -- Fearless Girdle
            25925, -- Terrorcloth Mantle
        },
    },
    {
        ['quests'] = {
            9927, -- Ruthless Cunning
            9928, -- Armaments for Deception
            9931, -- Returning the Favor
            9932, -- Body of Evidence
        },
        ['items'] = {
            25603, -- Lantresor's Warblade
            25605, -- Burning Blade Devotee's Cinch
            25606, -- Burning Blade Cultist Band
        },
    },
    {
        ['quests'] = {
            9971, -- Clues in the Thicket
        },
        ['items'] = {
            25926, -- Nexus-Stalker's Band
            25927, -- Consortium Cloak of the Quick
            25928, -- Ethereal Healing Pendant
            31724, -- Arakkoa Divining Rod
        },
    },
    {
        ['quests'] = {
            9971, -- Clues in the Thicket
            9990, -- Investigate Tuurem
        },
        ['items'] = {
            31782, -- Warpstalker Breastplate
            31783, -- Bloodfire Leggings
            31784, -- Ancient Terokkar Hood
            31785, -- Edge of Inevitability
        },
    },
    -- 10000:
    {
        ['quests'] = {
            10228, -- Ezekiel
            10231, -- What Book? I Don't See Any Book.
            10251, -- The Master's Grand Design?
            10252, -- Vision of the Dead
        },
        ['items'] = {
            29312, -- Cover of Righteous Fury
            29313, -- Earthbreaker's Greaves
            29314, -- Leggings of the Third Coin
            29315, -- Gloves of Penitence
        },
    },
    {
        ['quests'] = {
            10009, -- Crackin' Some Skulls
        },
        ['items'] = {
            25819, -- Breastplate of the Warbringer
            25820, -- Metallic Headband of Simm'onz
            25821, -- Leggings of Unending Assault
            25822, -- Watcher's Tunic
        },
    },
    {
        ['quests'] = {
            10590, -- Prove Your Hatred
        },
        ['items'] = {
            30696, -- Scourgebane
        },
    },
    {
        ['quests'] = {
            10642, -- A Ghost in the Machine
            10643, -- Harbingers of Shadowmoon
            10644, -- Teron Gorefiend - Lore and Legend
            10634, -- Divination: Gorefiend's Armor
            10635, -- Divination: Gorefiend's Cloak
            10636, -- Divination: Gorefiend's Truncheon
        },
        ['items'] = {
            31104, -- Evoker's Helmet of Second Sight
            31105, -- Overlord's Helmet of Second Sight
            31106, -- Stalker's Helmet of Second Sight
            31107, -- Shamanistic Helmet of Second Sight
            31109, -- Stealther's Helmet of Second Sight
            31110, -- Druidic Helmet of Second Sight
        },
    },
    {
        ['quests'] = {
            10662, -- The Hermit Smith (A)
            10663, -- The Hermit Smith (H)
        },
        ['items'] = {
            30788, -- Illidari-Bane Broadsword
            30789, -- Illidari-Bane Claymore
            30787, -- Illidari-Bane Mageblade
            31745, -- Illidari-Bane Dagger
        },
    },
    -- 11000:
    {
        ['quests'] = {
            11978, -- Into the Fold
            11983, -- Blood Oath of the Horde
        },
        ['items'] = {
            37956, -- Frontrunner's Blessed Handwraps,
            37998, -- Snowfall Reaver Boots
            38102, -- Stonepath Gauntlets
            38041, -- Westwind Waistband
            37957, -- Wastewind Bracers
            37999, -- Glade Wanderer Bracers
            38103, -- Petrified Bone Chestguard
            38042, -- Scourge Ghoul Collar
            37959, -- Wastewind Pauldrons
            38001, -- Snowfall Reaver Pauldrons
            38044, -- Anub'ar-Husk Shoulderguards
            38105, -- Stonepath Pauldrons
            38293, -- Suntouched Flowers
            38002, -- Honorborn Cloak
            38106, -- Petrified Bone Footguards
            37960, -- Oath Signet
            37963, -- Wastewind Handwraps
            38005, -- Spiritfury Bands
            38047, -- Belt of Ghostly Essence
            38109, -- Wraithshimmer Legplates
            38541, -- Implacable Zombie Crushers
            38543, -- Reinforced Traveler's Boots
            38540, -- Sandals of Chaos Resolution
            38542, -- Treads of the Valiant Struggle
            38178, -- Battlement Enforcer's Axe
            38177, -- Siege Captain's Gun
            38295, -- Idol of the Wastes
            38135, -- Mace of the Fallen Raven Priest
            38176, -- Stronghold Battlemace
            38134, -- Bloodtinged Spellblade
            38010, -- Scourgeslayer Cover
            38053, -- Wyrmfire Links
            38073, -- Will of the Red Dragonflight
            38114, -- Chestplate of the Ruby Champion
            44346, -- Warchief's Leggings of Wisdom
            44347, -- Warchief's Legguards of Heroism
            44349, -- Warchief's Legplates of Carnage
            44348, -- Warchief's Leggings of Valor
            44590, -- Warchief's Leggings of Foresight
            44583, -- Warchief's Legguards of Brutality
            44579, -- Medallion of Heroism
        },
    },
    -- 12000:
    {
        ['quests'] = {
            12056, -- Marked for Death: High Cultist Zangus
            12057, -- Koltira and the Language of Death
            12125, -- In Service of Blood
            12127, -- In Service of Frost
            12126, -- In Service of the Unholy
        },
        ['items'] = {
            37958, -- Wastewind Headcover - Item - WotLK Classic
            38000, -- Snowfall Reaver Breastplate - Item - WotLK Classic
            38043, -- Anub'ar-Husk Leggings - Item - WotLK Classic
            38104, -- Stonepath Helm - Item - WotLK Classic
        },
    },
    -- 13000:
    {
        ['quests'] = {
            13074, -- Hope Within the Emerald Nightmare
            13078, -- Dahlia's Tears
        },
        ['items'] = {
            44018, -- Bridenbrad's Sash
            44039, -- Signet of Bridenbrad
            44041, -- Belt of the Never-Forgotten
            44042, -- Chained Belt of Remembrance
            44043, -- Girdle of Eternal Memory
        },
    },
    {
        ['quests'] = {
            13242, -- Darkness Stirs
            13257, -- Herald of War
            13266, -- A Life Without Regret
            13267, -- The Battle For The Undercity
        },
        ['items'] = {
            44346, -- Warchief's Leggings of Wisdom
            44347, -- Warchief's Legguards of Heroism
            44349, -- Warchief's Legplates of Carnage
            44348, -- Warchief's Leggings of Valor
            44590, -- Warchief's Leggings of Foresight
            44583, -- Warchief's Legguards of Brutality
            44579, -- Medallion of Heroism
        },
    },
    
}

ScootsSpeedrun.extraQuestsDungeonSet2 = {
    ['all'] = {
        ['stage-2'] = {
            ['all'] = {
                8922, -- A Supernatural Device (A)
                8923, -- A Supernatural Device (H)
                8921, -- The Ectoplasmic Distiller
                8924, -- Hunting for Ectoplasm
                8925, -- A Portable Power Source
                8928, -- A Shifty Merchant
                8977, -- Return to Deliana
                8978, -- Return to Mokvar
            },
        },
        ['stage-3'] = {
            ['all'] = {
                8929, -- In Search of Anthion (A)
                8930, -- In Search of Anthion (H)
                8945, -- Dead Man's Plea
                8946, -- Proof of Life
                8947, -- Anthion's Strange Request
                8948, -- Anthion's Old Friend
                8949, -- Falrin's Vendetta
                8950, -- The Instigator's Enchantment
                9015, -- The Challenge
            },
        },
        ['stage-4'] = {
            ['onlyOne'] = {
                {
                    8962, -- Components of Importance
                    8963, -- Components of Importance
                    8964, -- Components of Importance
                    8965, -- Components of Importance
                },
                {
                    8966, -- The Left Piece of Lord Valthalak's Amulet
                    8967, -- The Left Piece of Lord Valthalak's Amulet
                    8968, -- The Left Piece of Lord Valthalak's Amulet
                    8969, -- The Left Piece of Lord Valthalak's Amulet
                },
                {
                    8985, -- More Components of Importance
                    8986, -- More Components of Importance
                    8987, -- More Components of Importance
                    8988, -- More Components of Importance
                },
                {
                    8989, -- The Right Piece of Lord Valthalak's Amulet
                    8990, -- The Right Piece of Lord Valthalak's Amulet
                    8991, -- The Right Piece of Lord Valthalak's Amulet
                    8992, -- The Right Piece of Lord Valthalak's Amulet
                },
            },
            ['all'] = {
                8960, -- Bodley's Unfortunate Fate (A)
                9032, -- Bodley's Unfortunate Fate (H)
                8961, -- Three Kings of Flame
                8970, -- I See Alcaz Island In Your Future...
                8994, -- Final Preparations
                8995, -- Mea Culpa, Lord Valthalak
                8996, -- Return to Bodley
                8997, -- Back to the Beginning (A)
                8998, -- Back to the Beginning (H)
            },
        },
    },
    ['DRUID'] = {
        ['stage-1'] = {
            ['quests'] = {
                8905, -- An Earnest Proposition (A)
                8913, -- An Earnest Proposition (H)
            },
            ['items'] = {
                22108, -- Feralheart Bracers
                22106, -- Feralheart Belt
                22110, -- Feralheart Gloves
                22107, -- Feralheart Boots
                22111, -- Feralheart Kilt
                22112, -- Feralheart Spaulders
                22109, -- Feralheart Cowl
                22113, -- Feralheart Vest
            },
        },
        ['stage-2'] = {
            ['quests'] = {
                8926, -- Just Compensation (A)
                8927, -- Just Compensation (H)
            },
            ['items'] = {
                22106, -- Feralheart Belt
                22110, -- Feralheart Gloves
                22107, -- Feralheart Boots
                22111, -- Feralheart Kilt
                22112, -- Feralheart Spaulders
                22109, -- Feralheart Cowl
                22113, -- Feralheart Vest
            },
        },
        ['stage-3'] = {
            ['quests'] = {
                8951, -- Anthion's Parting Words (A)
                9016, -- Anthion's Parting Words (H)
            },
            ['items'] = {
                22107, -- Feralheart Boots
                22111, -- Feralheart Kilt
                22112, -- Feralheart Spaulders
                22109, -- Feralheart Cowl
                22113, -- Feralheart Vest
            },
        },
        ['stage-4'] = {
            ['quests'] = {
                8999, -- Saving the Best for Last (A)
                9007, -- Saving the Best for Last (H)
            },
            ['items'] = {
                22109, -- Feralheart Cowl
                22113, -- Feralheart Vest
            },
        },
    },
    ['HUNTER'] = {
        ['stage-1'] = {
            ['quests'] = {
                8906, -- An Earnest Proposition (A)
                8914, -- An Earnest Proposition (H)
            },
            ['items'] = {
                22011, -- Beastmaster's Bindings
                22010, -- Beastmaster's Belt
                22015, -- Beastmaster's Gloves
                22061, -- Beastmaster's Boots
                22017, -- Beastmaster's Pants
                22016, -- Beastmaster's Mantle
                22013, -- Beastmaster's Cap
                22060, -- Beastmaster's Tunic
            },
        },
        ['stage-2'] = {
            ['quests'] = {
                8931, -- Just Compensation (A)
                8938, -- Just Compensation (H)
            },
            ['items'] = {
                22010, -- Beastmaster's Belt
                22015, -- Beastmaster's Gloves
                22061, -- Beastmaster's Boots
                22017, -- Beastmaster's Pants
                22016, -- Beastmaster's Mantle
                22013, -- Beastmaster's Cap
                22060, -- Beastmaster's Tunic
            },
        },
        ['stage-3'] = {
            ['quests'] = {
                8952, -- Anthion's Parting Words (A)
                9017, -- Anthion's Parting Words (H)
            },
            ['items'] = {
                22061, -- Beastmaster's Boots
                22017, -- Beastmaster's Pants
                22016, -- Beastmaster's Mantle
                22013, -- Beastmaster's Cap
                22060, -- Beastmaster's Tunic
            },
        },
        ['stage-4'] = {
            ['quests'] = {
                9000, -- Saving the Best for Last (A)
                9008, -- Saving the Best for Last (H)
            },
            ['items'] = {
                22013, -- Beastmaster's Cap
                22060, -- Beastmaster's Tunic
            },
        },
    },
    ['MAGE'] = {
        ['stage-1'] = {
            ['quests'] = {
                8907, -- An Earnest Proposition (A)
                8915, -- An Earnest Proposition (H)
            },
            ['items'] = {
                22063, -- Sorcerer's Bindings
                22062, -- Sorcerer's Belt
                22066, -- Sorcerer's Gloves
                22064, -- Sorcerer's Boots
                22067, -- Sorcerer's Leggings
                22068, -- Sorcerer's Mantle
                22065, -- Sorcerer's Crown
                22069, -- Sorcerer's Robes
            },
        },
        ['stage-2'] = {
            ['quests'] = {
                8932, -- Just Compensation (A)
                8939, -- Just Compensation (H)
            },
            ['items'] = {
                22062, -- Sorcerer's Belt
                22066, -- Sorcerer's Gloves
                22064, -- Sorcerer's Boots
                22067, -- Sorcerer's Leggings
                22068, -- Sorcerer's Mantle
                22065, -- Sorcerer's Crown
                22069, -- Sorcerer's Robes
            },
        },
        ['stage-3'] = {
            ['quests'] = {
                8953, -- Anthion's Parting Words (A)
                9018, -- Anthion's Parting Words (H)
            },
            ['items'] = {
                22064, -- Sorcerer's Boots
                22067, -- Sorcerer's Leggings
                22068, -- Sorcerer's Mantle
                22065, -- Sorcerer's Crown
                22069, -- Sorcerer's Robes
            },
        },
        ['stage-4'] = {
            ['quests'] = {
                9001, -- Saving the Best for Last (A)
                9014, -- Saving the Best for Last (H)
            },
            ['items'] = {
                22065, -- Sorcerer's Crown
                22069, -- Sorcerer's Robes
            },
        },
    },
    ['PALADIN'] = {
        ['stage-1'] = {
            ['quests'] = {
                8908, -- An Earnest Proposition
            },
            ['items'] = {
                22088, -- Soulforge Bracers
                22086, -- Soulforge Belt
                22090, -- Soulforge Gauntlets
                22087, -- Soulforge Boots
                22092, -- Soulforge Legplates
                22093, -- Soulforge Spaulders
                22089, -- Soulforge Breastplate
                22091, -- Soulforge Helm
            },
        },
        ['stage-2'] = {
            ['quests'] = {
                8933, -- Just Compensation
            },
            ['items'] = {
                22086, -- Soulforge Belt
                22090, -- Soulforge Gauntlets
                22087, -- Soulforge Boots
                22092, -- Soulforge Legplates
                22093, -- Soulforge Spaulders
                22089, -- Soulforge Breastplate
                22091, -- Soulforge Helm
            },
        },
        ['stage-3'] = {
            ['quests'] = {
                8954, -- Anthion's Parting Words
            },
            ['items'] = {
                22087, -- Soulforge Boots
                22092, -- Soulforge Legplates
                22093, -- Soulforge Spaulders
                22089, -- Soulforge Breastplate
                22091, -- Soulforge Helm
            },
        },
        ['stage-4'] = {
            ['quests'] = {
                9002, -- Saving the Best for Last
            },
            ['items'] = {
                22089, -- Soulforge Breastplate
                22091, -- Soulforge Helm
            },
        },
    },
    ['PRIEST'] = {
        ['stage-1'] = {
            ['quests'] = {
                8909, -- An Earnest Proposition (A)
                8916, -- An Earnest Proposition (H)
            },
            ['items'] = {
                22079, -- Virtuous Bracers
                22078, -- Virtuous Belt
                22081, -- Virtuous Gloves
                22084, -- Virtuous Sandals
                22085, -- Virtuous Skirt
                22082, -- Virtuous Mantle
                22080, -- Virtuous Crown
                22083, -- Virtuous Robe
            },
        },
        ['stage-2'] = {
            ['quests'] = {
                8934, -- Just Compensation (A)
                8940, -- Just Compensation (H)
            },
            ['items'] = {
                22078, -- Virtuous Belt
                22081, -- Virtuous Gloves
                22084, -- Virtuous Sandals
                22085, -- Virtuous Skirt
                22082, -- Virtuous Mantle
                22080, -- Virtuous Crown
                22083, -- Virtuous Robe
            },
        },
        ['stage-3'] = {
            ['quests'] = {
                8955, -- Anthion's Parting Words (A)
                9019, -- Anthion's Parting Words (H)
            },
            ['items'] = {
                22084, -- Virtuous Sandals
                22085, -- Virtuous Skirt
                22082, -- Virtuous Mantle
                22080, -- Virtuous Crown
                22083, -- Virtuous Robe
            },
        },
        ['stage-4'] = {
            ['quests'] = {
                9003, -- Saving the Best for Last (A)
                9009, -- Saving the Best for Last (H)
            },
            ['items'] = {
                22080, -- Virtuous Crown
                22083, -- Virtuous Robe
            },
        },
    },
    ['ROGUE'] = {
        ['stage-1'] = {
            ['quests'] = {
                8910, -- An Earnest Proposition (A)
                8917, -- An Earnest Proposition (H)
            },
            ['items'] = {
                22004, -- Darkmantle Bracers
                22006, -- Darkmantle Gloves
                22002, -- Darkmantle Belt
                22003, -- Darkmantle Boots
                22007, -- Darkmantle Pants
                22008, -- Darkmantle Spaulders
                22005, -- Darkmantle Cap
                22009, -- Darkmantle Tunic
            },
        },
        ['stage-2'] = {
            ['quests'] = {
                8935, -- Just Compensation (A)
                8941, -- Just Compensation (H)
            },
            ['items'] = {
                22006, -- Darkmantle Gloves
                22002, -- Darkmantle Belt
                22003, -- Darkmantle Boots
                22007, -- Darkmantle Pants
                22008, -- Darkmantle Spaulders
                22005, -- Darkmantle Cap
                22009, -- Darkmantle Tunic
            },
        },
        ['stage-3'] = {
            ['quests'] = {
                8956, -- Anthion's Parting Words (A)
                9020, -- Anthion's Parting Words (H)
            },
            ['items'] = {
                22003, -- Darkmantle Boots
                22007, -- Darkmantle Pants
                22008, -- Darkmantle Spaulders
                22005, -- Darkmantle Cap
                22009, -- Darkmantle Tunic
            },
        },
        ['stage-4'] = {
            ['quests'] = {
                9004, -- Saving the Best for Last (A)
                9010, -- Saving the Best for Last (H)
            },
            ['items'] = {
                22005, -- Darkmantle Cap
                22009, -- Darkmantle Tunic
            },
        },
    },
    ['SHAMAN'] = {
        ['stage-1'] = {
            ['quests'] = {
                8918, -- An Earnest Proposition
            },
            ['items'] = {
                22095, -- Bindings of The Five Thunders
                22098, -- Cord of The Five Thunders
                22099, -- Gauntlets of The Five Thunders
                22096, -- Boots of The Five Thunders
                22100, -- Kilt of The Five Thunders
                22101, -- Pauldrons of The Five Thunders
                22097, -- Coif of The Five Thunders
                22102, -- Vest of The Five Thunders
            },
        },
        ['stage-2'] = {
            ['quests'] = {
                8942, -- Just Compensation
            },
            ['items'] = {
                22098, -- Cord of The Five Thunders
                22099, -- Gauntlets of The Five Thunders
                22096, -- Boots of The Five Thunders
                22100, -- Kilt of The Five Thunders
                22101, -- Pauldrons of The Five Thunders
                22097, -- Coif of The Five Thunders
                22102, -- Vest of The Five Thunders
            },
        },
        ['stage-3'] = {
            ['quests'] = {
                8957, -- Anthion's Parting Words
            },
            ['items'] = {
                22096, -- Boots of The Five Thunders
                22100, -- Kilt of The Five Thunders
                22101, -- Pauldrons of The Five Thunders
                22097, -- Coif of The Five Thunders
                22102, -- Vest of The Five Thunders
            },
        },
        ['stage-4'] = {
            ['quests'] = {
                9011, -- Saving the Best for Last
            },
            ['items'] = {
                22097, -- Coif of The Five Thunders
                22102, -- Vest of The Five Thunders
            },
        },
    },
    ['WARLOCK'] = {
        ['stage-1'] = {
            ['quests'] = {
                8911, -- An Earnest Proposition (A)
                8919, -- An Earnest Proposition (H)
            },
            ['items'] = {
                22071, -- Deathmist Bracers
                22070, -- Deathmist Belt
                22077, -- Deathmist Wraps
                22076, -- Deathmist Sandals
                22072, -- Deathmist Leggings
                22073, -- Deathmist Mantle
                22074, -- Deathmist Mask
                22075, -- Deathmist Robe
            },
        },
        ['stage-2'] = {
            ['quests'] = {
                8936, -- Just Compensation (A)
                8943, -- Just Compensation (H)
            },
            ['items'] = {
                22070, -- Deathmist Belt
                22077, -- Deathmist Wraps
                22076, -- Deathmist Sandals
                22072, -- Deathmist Leggings
                22073, -- Deathmist Mantle
                22074, -- Deathmist Mask
                22075, -- Deathmist Robe
            },
        },
        ['stage-3'] = {
            ['quests'] = {
                8958, -- Anthion's Parting Words (A)
                9021, -- Anthion's Parting Words (H)
            },
            ['items'] = {
                22076, -- Deathmist Sandals
                22072, -- Deathmist Leggings
                22073, -- Deathmist Mantle
                22074, -- Deathmist Mask
                22075, -- Deathmist Robe
            },
        },
        ['stage-4'] = {
            ['quests'] = {
                9005, -- Saving the Best for Last (A)
                9012, -- Saving the Best for Last (H)
            },
            ['items'] = {
                22074, -- Deathmist Mask
                22075, -- Deathmist Robe
            },
        },
    },
    ['WARRIOR'] = {
        ['stage-1'] = {
            ['quests'] = {
                8912, -- An Earnest Proposition (A)
                8920, -- An Earnest Proposition (H)
            },
            ['items'] = {
                21996, -- Bracers of Heroism
                21994, -- Belt of Heroism
                21998, -- Gauntlets of Heroism
                21995, -- Boots of Heroism
                22000, -- Legplates of Heroism
                22001, -- Spaulders of Heroism
                21999, -- Helm of Heroism
                21997, -- Breastplate of Heroism
            },
        },
        ['stage-2'] = {
            ['quests'] = {
                8937, -- Just Compensation (A)
                8944, -- Just Compensation (H)
            },
            ['items'] = {
                21994, -- Belt of Heroism
                21998, -- Gauntlets of Heroism
                21995, -- Boots of Heroism
                22000, -- Legplates of Heroism
                22001, -- Spaulders of Heroism
                21999, -- Helm of Heroism
                21997, -- Breastplate of Heroism
            },
        },
        ['stage-3'] = {
            ['quests'] = {
                8959, -- Anthion's Parting Words (A)
                9022, -- Anthion's Parting Words (H)
            },
            ['items'] = {
                21995, -- Boots of Heroism
                22000, -- Legplates of Heroism
                22001, -- Spaulders of Heroism
                21999, -- Helm of Heroism
                21997, -- Breastplate of Heroism
            },
        },
        ['stage-4'] = {
            ['quests'] = {
                9006, -- Saving the Best for Last (A)
                9013, -- Saving the Best for Last (H)
            },
            ['items'] = {
                21999, -- Helm of Heroism
                21997, -- Breastplate of Heroism
            },
        },
    },
}

ScootsSpeedrun.extraQuestsDungeonSet2.DEATHKNIGHT = ScootsSpeedrun.extraQuestsDungeonSet2.WARRIOR